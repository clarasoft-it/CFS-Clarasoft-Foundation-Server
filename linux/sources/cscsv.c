  ...
  while(1) {
    ...
    if (c == '"') {
          ...
          case '"':
            ...
            if (c == '"') {
              ...
              if (c == This->separator) {
                ...
                if (feof(This->file)) {
                  // This means we have a separator at EOF, 
                  // so we have an empty toke  
                  This->lineBuffer[0] = 0;
                  CSLIST_Insert(record, This->lineBuffer, 
                            strlen(This->lineBuffer) + 1, CSLIST_BOTTOM);
                  return CS_SUCCESS;
                  ...
                  if (c == '\r' || c == '\n') {
                    // This means we are at the end of the line, so 
                    // we have an empty token
                    This->lineBuffer[0] = 0;
                    CSLIST_Insert(record, This->lineBuffer, 
                            strlen(This->lineBuffer) + 1, CSLIST_BOTTOM);
                    return CS_SUCCESS;
      ...
      if (c == This->separator) {
        ...
        if (feof(This->file)) {
          // This means we have an empy token at EOF
          This->lineBuffer[0] = 0;
          CSLIST_Insert(record, This->lineBuffer, 
                      strlen(This->lineBuffer) + 1, CSLIST_BOTTOM);
          return CS_SUCCESS;
